...
module.exports.run = async (client, message, args, data) => {
    ...
    await member.roles.remove(muteRole.id).then(async () => {
        user.send(`Vous avez été unmute du serveur **${message.guild.name}** par ${message.author}. Raison : ${reason}`);
        ...
        if(dbUser.tempmutes.some(mute => mute.guildID === message.guild.id)) {
            dbUser.tempmutes = dbUser.tempmutes.filter(mute => mute.guildID !== message.guild.id);
